<!-- Need to replace this with a main page -->
# Introduction

## Getting Started

- [What is Distributed Chaos?](./getting-started/what-is-dc.md)
- [Starting a Group](./getting-started/starting-a-group.md)

## Framework Guidelines

- [Overview](./framework/overview.md)
- [Core Principles](./framework/principles.md)
- [Group Structure](./framework/structure.md)

## Running Groups

- [Meeting Formats](./running-groups/meeting-formats.md)

## Community Resources

- [Communication Channels](./community/communication.md)
- [Sponsorship](./community/sponsorship.md)
- [Tools and Templates](./community/tools.md)

## Contributing

- [How to Contribute](./contributing/how-to.md)
- [Documentation Style Guide](./contributing/style-guide.md)
